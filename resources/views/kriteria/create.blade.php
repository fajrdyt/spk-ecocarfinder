...
@section('title', 'Tambah Kriteria - EcoCarFinder')

@section('content')
<div class="container-fluid">
    <div class="mb-4">
        <h2><i class="fas fa-plus-circle me-2"></i>Tambah Kriteria</h2>
                    ...
                    <form action="{{ route('kriteria.store') }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('kode_kriteria') is-invalid @enderror" 
                                   name="kode_kriteria" value="{{ old('kode_kriteria') }}" 
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('jenis') is-invalid @enderror" name="jenis" required>
                                ...
                                <option value="benefit" {{ old('jenis') == 'benefit' ? 'selected' : '' }}>Benefit (Semakin besar semakin baik)</option>
                                <option value="cost" {{ old('jenis') == 'cost' ? 'selected' : '' }}>Cost (Semakin kecil semakin baik)</option>
                            </select>
                            @error('jenis')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
